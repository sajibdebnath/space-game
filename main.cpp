...
int NOF=6;              //Number of enemies per frame
...
class enemy             //enemy class
{
	...
	{
		glColor3f(0.93,0.035,0.035);//RGB:(237, 9 ,9)
		glBegin(GL_QUADS);
			glVertex2f(x,y);
...
void showstars()  //renders the start;
{
	...
	for(i=0;i<n;i++)
	{
		...
		{
			s[i].y=800;     //initial y position 800
			s[i].x=rand()%800; //initial x position
		}
	}
...
int bulletspeed=30;  //this value will be added to the y of bullet
...
rendertext(10,780,ss);
rendertext(10,760,sf);
...
void keyboard(unsigned char key, int x, int y)
{
	switch(key)
	{
		...
		case 32:	if(ship.alive)
					{ship.shoot=1;
                    NOB++;}    //Bumber of Bullets++
                    //sndPlaySound("/home/sajib/Music/sound.wav", SND_ASYNC);
		break;
		case 'D':
		case 'd':
